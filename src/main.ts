...
const MENU_MLOG: MenuAction[] = [
  ...
  { id: "restructure", text: "重建控制流（暴力反编译）" },
...
/** 把控制流重建结果排成可读报告 */
function formatFlowReport(res: DecompileResult, filename: string): string {
  const out: string[] = [];
  out.push("# 控制流重建（暴力反编译）");
  out.push(
    `# 来源：${filename} · 输入 ${res.lines} 行 · 迭代 ${res.iterations} 轮 · 候选 ${res.candidates} 个`,
  );
  out.push("# 按质量排序，loss 越小越好；标签为 定义数/引用数");
  for (const c of res.cases) {
    out.push("");
    out.push(
      `# ---------- case ${c.index} · loss ${c.loss.toFixed(5)} · 标签 ${c.labels_def}/${c.labels_used} ----------`,
    );
    out.push(c.logic.trimEnd());
  }
  out.push("");
  return out.join("\n");
}
...
class AcodePlugin {
  ...
  private renderPanel(container: HTMLElement): void {
    ...
    if (kind === "mdtlbl") {
      ...
      btn("重建控制流", () => void this.actionRestructure());
  ...
  private async showActionsMenu(): Promise<void> {
    ...
    const actions: Record<string, () => void> = {
      ...
      restructure: () => void this.actionRestructure(),
  ...
  /**
   * 暴力重建控制流：把一串 jump / @counter 归约成 do-while / if-else / gswitch 结构
   * 内核为上游 mlog-decompiler（已编进 wasm，离线可用）
   */
  private async actionRestructure(): Promise<void> {
    const active = this.getActiveFile();
    if (!active) return;
    if (!/\.(mlog|logic)$/i.test(active.filename)) {
      window.toast("控制流重建仅支持 .mlog / .logic 文件", 3000);
      return;
    }
    const loader = this.showLoader("正在重建控制流...");
    try {
      const res = await decompileMlog(active.content);
      loader?.destroy?.();
      if (!res.ok) {
        acode.alert("控制流重建失败", res.error.slice(0, 2000));
        return;
      }
      if (!res.cases.length) {
        window.toast("没有可重建的跳转结构", 3000);
        return;
      }
      const name = active.filename.replace(/\.(mlog|logic)$/i, "") + "-flow.txt";
      void this.openInNewFile(name, formatFlowReport(res, active.filename));
      window.toast(`控制流重建完成：${res.cases.length} 个候选`, 3000);
    } catch (e) {
      loader?.destroy?.();
      acode.alert("控制流重建出错", String((e as Error).message));
    }
  }
  ...
  private async actionDiagnose(): Promise<void> {
    ...
    try {
      const probe = await decompileMlog("set i 0\nop add i i 1\njump 1 lessThan i 3");
      lines.push(
        `控制流重建: ${probe.ok ? `就绪（${probe.cases.length} 个候选）` : `失败 ${probe.error}`}`,
      );
    } catch (e) {
      lines.push(`控制流重建: 异常 ${(e as Error).message}`);
    }
