...
use syntax::{CompileMeta, CompileMetaExtends, Expand, Meta};
use tag_code::{
...
// ---------- 暴力重建控制流（内置上游 mlog-decompiler） ----------

/// 输入逻辑行数上限（超出直接拒绝，避免 wasm 内存被撑爆）
const DECOMPILE_MAX_LINES: usize = 3000;
/// 迭代次数上限
const DECOMPILE_MAX_ITER: usize = 64;
/// 每轮保留候选数上限
const DECOMPILE_MAX_LIMIT: usize = 1500;
/// 输出候选个数上限
const DECOMPILE_MAX_OUT: usize = 10;

fn decompile_err(msg: &str) -> String {
    format!(
        "{{\"ok\":false,\"error\":\"{}\",\"iterations\":0,\"cases\":[]}}",
        json_escape(msg)
    )
}

/// 暴力重建控制流：把一串 jump / @counter 归约成 break、while、if-else 等结构化伪代码
///
/// - `iterations`：迭代次数（钳制到 1..=64，越大质量越高、越慢）
/// - `limit`：每轮保留的最优候选数（钳制到 20..=1500，越大越慢越占内存）
/// - `out_limit`：输出候选个数（钳制到 1..=10）
/// - `guidance`：指导模式，更激进的重建（嵌套更深、质量可能偏低）
///
/// 返回 JSON：`{"ok":bool,"error":str,"iterations":usize,"cases":[{...}]}`，
/// 每个 case 为
/// `{"index":usize,"loss":f32,"labels_def":usize,"labels_used":usize,"logic":str,"bang":str}`。
/// `logic` 为逻辑风格伪代码（`:x` 格式），`bang` 为 `skip/while/gswitch` 风格的原始格式。
/// 不会抛 JS 异常。
#[wasm_bindgen]
pub fn decompile_mlog(
    source: &str,
    iterations: usize,
    limit: usize,
    out_limit: usize,
    ...
    use decompiler::quality::Loss;

    console_error_panic_hook::set_once();

    let iterations = iterations.clamp(1, DECOMPILE_MAX_ITER);
    let limit = limit.clamp(20, DECOMPILE_MAX_LIMIT);
    let out_limit = out_limit.clamp(1, DECOMPILE_MAX_OUT);

    let mut lines = match logic_parse(source) {
        Ok(it) => it,
        Err(e) => return decompile_err(&format!("输入不是合法的 mlog —— {e}")),
    };
    let raw_lines = lines.lines().len();
    if raw_lines > DECOMPILE_MAX_LINES {
        return decompile_err(&format!(
            "逻辑行数 {raw_lines} 超过上限 {DECOMPILE_MAX_LINES}，请先裁掉无关部分"
        ));
    }
    ...
    let reduces = decompiler::make::make_reduce(lines.lines().iter().map(|x| &x.value));

    let mut finder = decompiler::Finder {
        current: std::iter::once(reduces.into()).collect(),
        losses_cache: vec![],
        limit,
        guidance,
    };

    let mut itering = 0usize;
    ...
    loop {
        ...
        // 结果不再变化即提前收敛
        if Some((happy, limite, raw_len)) == prev_limite || itering >= iterations {
            break;
        }
    ...
    for (i, reduce) in sorted.iter().take(out_limit).enumerate() {
        if i > 0 {
            cases.push(',');
        }
        let loss = reduce.loss();
        let labels_def = decompiler::walk::label_defs(reduce);
        let labels_used = decompiler::walk::label_usages(reduce);
        let logic = format!("{reduce:x}");
        let bang = format!("{reduce}");
        cases.push_str(&format!(
            "{{\"index\":{i},\"loss\":{loss},\"labels_def\":{labels_def},\
             \"labels_used\":{labels_used},\"logic\":\"{}\",\"bang\":\"{}\"}}",
            json_escape(&logic),
            json_escape(&bang),
        ));
    }
    ...
    format!(
        "{{\"ok\":true,\"error\":\"\",\"iterations\":{itering},\
         \"lines\":{raw_lines},\"candidates\":{total},\"cases\":[{cases}]}}"
    )
}
